        ...
        *pending_switch = pending_switch.filter(|(slot, block_id)| {
            ...
            loop {
                ...
                let parent_slot = slot_meta
                    .parent_slot
                    .expect("Full slots must have parent_slot");

                if bank_forks.read().unwrap().block_id(parent_slot)
                    == Some(slot_meta.parent_block_id)
                ...
                ancestor_block_id = slot_meta.parent_block_id;
            }
            ...
            for (slot, location) in blocks_to_switch.into_iter() {
                ...
                if let Err(err) = blockstore.switch_block_from_alternate(slot, location) {
                    warn!("{my_pubkey}: Failed to switch {slot} from {location:?}: {err:?}");
                    continue;
                }
