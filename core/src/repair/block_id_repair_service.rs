...
impl BlockIdRepairService {
    ...
    fn process_repair_event(
        ...
        match event {
            RepairEvent::FetchBlock { slot, block_id } => {
                ...
                if let Ok(Some(location)) = blockstore.get_block_location(slot, block_id) {
                ...
                match blockstore.get_double_merkle_root(slot, BlockLocation::Original) {
                    ...
                    Ok(Some(turbine_block_id)) if turbine_block_id != block_id => {
                    ...
                    Err(err) => {
                        warn!(
                            "{my_pubkey}: FetchBlock: Failed checking turbine block for slot \
                             {slot}: {err:?}"
                        );
                    }
                }
    ...
    /// Helper to fetch the parent block for a block we already have
    ...
    fn queue_fetch_parent_block(
        ...
        let slot_meta = blockstore
            .meta_from_location(slot, location)
            .unwrap()
            .expect("SlotMeta must be populated for full blocks");

        state.pending_repair_events.push(RepairEvent::FetchBlock {
            slot: slot_meta
                .parent_slot
                .expect("parent slot must be present for full blocks"),
...
mod tests {
    ...
    fn test_process_block_id_repair_response_parent_fec_set_count() {
        ...
        assert_eq!(state.pending_repair_requests.len(), fec_set_count as usize);
