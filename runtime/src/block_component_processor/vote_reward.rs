...
impl<'a> RewardState<'a> {
    ...
    fn update_leader(&self, vote_state: &mut VoteState, leader_reward: u64) {
...
fn get_migration_epoch(bank: &Bank) -> Option<Epoch> {
    let genesis_cert = bank.get_alpenglow_genesis_certificate()?;
...
fn increment_credits(
    ...
    let Some(entry) = epoch_credits.last_mut() else {
        ...
        epoch_credits.push((epoch, new_credits, 0));
        return;
    };
    ...
    if *entry == AG_MIGRATION_EPOCH_CREDIT {
        ...
        epoch_credits.push((
            epoch,
            new_credits.saturating_add(final_tower_credits),
            final_tower_credits,
        ));
    ...
    let entry = (
        epoch,
        new_credits.saturating_add(*final_credits),
        *final_credits,
    );
