...
pub(super) fn calc_vote_rewards_update_vote_states(
    ...
    if let Some((signers, _)) = final_cert_input {
        vote_account_updates.extend(
            signers.iter().map(|validator_vote_pubkey| {
    ...
    for (validator_vote_pubkey, vote_account_update) in vote_account_updates {
        let Some((_, current_slot_account)) = current_vote_accounts.get(&validator_vote_pubkey)
            ...
            if vote_account_update.reward.is_some() {
                info!(
                    "validator {validator_vote_pubkey} was present for reward_slot {reward_slot} \
...
fn update_vote_state(
    ...
    if let Some((signers, final_slot)) = final_cert_input {
