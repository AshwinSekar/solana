...
mod tests {
    ...
    enum PayType {
        Both { ag_credits: u64, tower_credits: u64 },
        Tower(u64),
        NoneWithMarker,
        NoneNoMarker,
    }
    ...
    impl PayType {
        fn tower(&self) -> u64 {
            match self {
                ...
                Self::Tower(t) => *t,
                Self::NoneWithMarker => 0,
                Self::NoneNoMarker => 0,
            }
        }

        fn ag(&self) -> Option<u64> {
            match self {
                Self::Both { ag_credits, .. } => Some(*ag_credits),
                Self::Tower(_) => Some(0),
                Self::NoneWithMarker => Some(0),
                Self::NoneNoMarker => None,
            }
        }
    }
    ...
    impl State {
        ...
        fn validate_stakers(
            ...
            for staker_pubkey in self.stakers.get(voter_pubkey).unwrap().iter() {
                ...
                let stake_weighted_ag = self.pay_type.ag().unwrap_or(0) * stake / validator_stake;
    ...
    #[test_matrix([true, false], [1_000, 5_000], [0, 10], [PayType::Both{ag_credits: 1023, tower_credits:532}, PayType::Tower(383), PayType::NoneWithMarker, PayType::NoneNoMarker])]
