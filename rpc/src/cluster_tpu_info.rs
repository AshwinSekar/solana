...
impl ClusterTpuInfo {
    ...
    fn alpenglow_leader_pubkeys(&self, current_slot: Slot, max_count: u64) -> Vec<Pubkey> {
        (0..max_count)
            .filter_map(|i| {
                let target_slot =
                    current_slot.saturating_add(i.saturating_mul(NUM_CONSECUTIVE_LEADER_SLOTS));
...
mod test {
    ...
    fn test_get_leader_tpus_uses_commitment_slot_after_alpenglow() {
        ...
        let completed_alpenglow_slot = (1..64)
            .find(|slot| {
                solana_runtime::leader_schedule_utils::slot_leader_at(*slot, &bank).unwrap()
                    != solana_runtime::leader_schedule_utils::slot_leader_at(
        ...
        let completed_slot_leader =
            solana_runtime::leader_schedule_utils::slot_leader_at(completed_alpenglow_slot, &bank)
                .unwrap();
        let target_slot_leader = solana_runtime::leader_schedule_utils::slot_leader_at(
            completed_alpenglow_slot.saturating_add(1),
            &bank,
        )
        .unwrap();
