                    ...
                    if !matches!(location, BlockLocation::Original) {
                        error!(
                            "Shreds being inserted to the alternate column {location:?} for slot \
                             {} have mismatching ParentInfo.
                            Note that these shreds have reached a confirmation threshold, and were \
                             fetched by block id repair which
                            does pre verification via merkle proofs before ingest. For a mismatch \
                             to occur something has gone
                            disasterously wrong and we might not be able to recover. Marking the \
                             slot as dead.",
                            shred.slot(),
                        );
                    }
