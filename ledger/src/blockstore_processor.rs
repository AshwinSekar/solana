...
pub fn process_entries_for_tests(
    ...
    let entry::HashVerifiedTransactions {
        entries,
        unverified_signatures,
    } = entry::verify_transaction_hashes(
        entries,
        num_txs,
        &replay_tx_thread_pool,
        verify_transaction,
...
fn confirm_slot_entries(
    ...
    let entry::HashVerifiedTransactions {
        entries,
        unverified_signatures,
    } = match entry::verify_transaction_hashes(
        entries,
        num_txs,
        transaction_hash_verify_thread_pool(),
        verify_transaction_hash,
    ) {
        Ok(hash_verification_result) => hash_verification_result,
        Err(err) => {
            warn!(
