        ...
        match entry {
            ...
            CliEpochVotingHistory::Alpenglow(ag) => {
                ...
                writeln!(
                    ...
                    ag.lamports_earned,
                    ag.lamports_earned as f64 / 1_000_000_000_f64,
                )?;
            }
        }
    }
...
pub fn get_epoch_history(
    ...
    let mut ret = vec![];
    let migration_epoch = match ag_genesis_cert {
        None => None,
        Some(cert) => {
    ...
    for entry in vote_state.epoch_credits.iter().copied() {
        ...
        let in_tower = match migration_epoch {
            Some(migration_epoch) => {
                epoch < migration_epoch || (epoch == migration_epoch && !marker_seen)
            }
            None => true,
        };
        let entry = if in_tower {
